...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        "Привет. Планируешь закупку ИТ-оборудования или лицензии, "
        "но не знаешь, как правильно оформить договорные отношения с контрагентом? "
        "Я помогу :) Готов ответить на несколько вопросов?"
    )
...
async def send_template_offer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text_6 = (
        "Поздравляем! Для такого предмета договора в компании используется стандартный подход.\n"
        "Ниже мы сможем выдать вам типовую форму договора с полями для заполнения.\n\n"
        "Если контрагент будет готов подписать договор без существенных изменений, "
        "такой проект можно будет передать юристам для быстрого согласования.\n\n"
        "Подходит ли вам типовая форма договора?"
    )
